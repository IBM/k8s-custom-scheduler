...
func (f *ConfigFactory) CreateFromKeys(predicateKeys, priorityKeys sets.String, extenders []algorithm.SchedulerExtender) (*scheduler.Config, error) {
	...
	return &scheduler.Config{
		...
		PodWriter:           &podWriter{Client: f.client},
...
func (pw *podWriter) UpdatePod(oldPod, newPod *v1.Pod) error {
	pw.mutex.Lock()
	defer pw.mutex.Unlock()
	graceperiod := int64(0)
	if oldPod.Labels["task-type"] == "Nimbix" {
		glog.V(2).Infof("Deleting pod %s/%s as it could not be scheduled", oldPod.Namespace, oldPod.Name)
		err := pw.Client.CoreV1().Pods(oldPod.Namespace).Delete(oldPod.Name, &metav1.DeleteOptions{
		...
		glog.V(2).Infof("Creating new pod %s/%s", newPod.Namespace, newPod.Name)
		pod, err := pw.Client.CoreV1().Pods(oldPod.Namespace).Create(newPod)
		if err != nil {
			return err
		}
		glog.V(2).Infof("Created new pod %s/%s", pod.Namespace, pod.Name)
	}
	return nil
}
