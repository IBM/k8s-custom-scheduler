...
// PodWriter delete the pod and create a new pod based on the passed pod.
...
type Config struct {
	...
	// PodWriter is only used to schedule Pods those can be run on Nimbix.
	PodWriter PodWriter
...
func (sched *Scheduler) scheduleOne() {
	...
	if len(suggestedHost) == 0 {
		if sched.checkPodLabelForNimbix(pod) {
			if len(pod.Spec.Containers) > 1 {
				glog.Errorf("Pods expected to be run in Nimbix must have no more than one container")
				return
			}

			// Modify the Pod specification to run it in any node with nimbix access.
			pod.DeletionTimestamp = nil
			newPod := sched.modifyPodForNimbix(pod)

			if updateErr := sched.config.PodWriter.UpdatePod(pod, newPod); updateErr != nil {
...
// Check whether the pod contains label task-type "Nimbix"
func (sched *Scheduler) checkPodLabelForNimbix(pod *v1.Pod) bool {
	if pod.Labels["task-type"] == "Nimbix" {
		return true
	}
...
func (sched *Scheduler) modifyPodForNimbix(oldPod *v1.Pod) *v1.Pod {
			...
			Containers: []v1.Container{
				{
					...
					Env:                      append(oldContainer.Env, newEnv...),
